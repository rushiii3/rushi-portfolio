...
> 🖼️ **Diagram 1 — The CIA Triad**
> _Editor: An equilateral triangle. Top vertex (blue): "Confidentiality" with "Keep it private" beneath. Bottom-left vertex (green): "Integrity" with "Keep it accurate" beneath. Bottom-right vertex (orange): "Availability" with "Keep it accessible" beneath. Each side of the triangle has a bidirectional arrow. Center of the triangle reads "Information Security" in bold. Clean minimal flat design, no shadows._
...
> 🖼️ **Diagram 2 — Identification → Authentication → Authorization**
> _Editor: Three boxes connected by right-pointing arrows in a horizontal line. Box 1 (grey): "Identification" bold, subtitle "Who are you?", example below "Username / Email". Box 2 (orange): "Authentication" bold, subtitle "Prove it.", example below "Password + MFA". Box 3 (green): "Authorization" bold, subtitle "What can you do?", example below "Role-based permissions". Below the entire flow, a note in smaller text: "Each step is required — skipping one breaks the chain". Arrows between boxes are thick and clearly directional._
...
> 🖼️ **Diagram 3 — Five Authentication Factors**
> _Editor: Five horizontal rows, each a card. Row 1 (blue, brain icon): "Something You Know — Password, PIN". Row 2 (purple, card icon): "Something You Have — Smart Card, Token, Authenticator App". Row 3 (green, fingerprint icon): "Something You Are — Fingerprint, Retina, Face". Row 4 (orange, map pin icon): "Somewhere You Are — Approved IP, GPS Location". Row 5 (teal, keyboard icon): "Something You Do — Keystroke Pattern, Signature". Right side of each row: a small strength bar (1–5 filled circles) — Know=1, Have=3, Are=4, Where=2, Do=3. Label the strength column "Standalone Strength"._
...
> 🖼️ **Diagram 4 — MFA Factor Combinations**
> _Editor: A table-style diagram with 5 rows and 3 columns. Header row: "Combination", "Factors Used", "Security Level". Rows: (1) Password + Hardware Token — Know + Have — High (green). (2) Password + Authenticator App — Know + Have — High (green). (3) Password + SMS Code — Know + Have — Medium (yellow, with note "SIM swap risk"). (4) Password + Fingerprint — Know + Are — Very High (dark green). (5) Smart Card + PIN — Have + Know — High (green). Security level shown as colored badge. Bottom note: "Two factors from same category ≠ MFA"._
...
- Link backward from Part 1: Information Security Concepts (/blog/information-security-concepts)
  → CIA triad operationalizes the prevention/detection/recovery goals
- Link forward to Part 3: Cryptography Fundamentals (/blog/cryptography-fundamentals)
  → encryption enforces confidentiality, hashing enforces integrity — directly from CIA triad
- Link backward from future posts on zero trust architecture
  → zero trust is the application of authentication/authorization principles at every layer
- Link backward from future posts on PKI and digital certificates
